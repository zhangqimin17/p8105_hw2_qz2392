...
The dimension of the resulting dataset is ```r nrow(fivethirtyeight)``` rows and ```r ncol(fivethirtyeight)``` columns. The range of years is ```r max(pull(fivethirtyeight, year)) - min(pull(fivethirtyeight, year))``` years. The names of key variables are 'president', 'rep_gop', 'rep_dem' and 'unemployment_rate', because the president and the ratio of republican govenors to democratic govenors can reflect to the power of a specific party, and the unemployment rate can reflect the economy status.
